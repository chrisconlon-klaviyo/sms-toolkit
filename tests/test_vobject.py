...
from sms_toolkit.vobject import base, iCalendar, icalendar
from sms_toolkit.vobject.base import ContentLine, ParseError
from sms_toolkit.vobject.base import __behaviorRegistry as behavior_registry
from sms_toolkit.vobject.base import parseLine, readComponents, textLineToContentLine
from sms_toolkit.vobject.change_tz import change_tz
...
class TestCalendarSerializing(unittest.TestCase):
    ...
    def test_scratchbuild(self):
        ...
        cal.vevent.add("created").value = datetime.datetime(
            ...
            1,
            10,
            tzinfo=dateutil.tz.tzical("test_files/timezones.ics").get("US/Pacific"),
        )
        cal.vevent.add("uid").value = "Not very random UID"
        cal.vevent.add("dtstamp").value = datetime.datetime(
            2017, 6, 26, 0, tzinfo=tzutc()
        )
    ...
    def test_multiline(self):
        ...
        self.assertEqual(
            category.serialize().strip(), "CATEGORIES:Random category,Other category"
        )
    ...
    def test_semicolon_separated(self):
        ...
        self.assertEqual(
            request_status.serialize().strip(), "REQUEST-STATUS:5.1;Service unavailable"
        )
...
class TestVTodo(unittest.TestCase):
    ...
    def test_vtodo(self):
        ...
        self.assertEqual(
            obj.vtodo.completed.serialize()[0:23], "COMPLETED:20150505T1330"
        )
...
class TestVobject(unittest.TestCase):
    ...
    def test_readComponents(self):
        ...
        self.assertEqual(
            str(cal.vevent.summary), "<SUMMARY{'BLAH': ['hi!']}Bastille Day Party>"
        )
...
class TestGeneralFileParsing(unittest.TestCase):
    ...
    def test_bad_line(self):
        ...
        self.assertEqual(
            str(newcal.vevent.x_bad_underscore), "<X-BAD-UNDERSCORE{}TRUE>"
        )
...
class TestIcalendar(unittest.TestCase):
    ...
    def test_regexes(self):
        ...
        """
        self.assertEqual(
            re.findall(base.patterns["name"], "12foo-bar:yay"), ["12foo-bar", "yay"]
        )
        self.assertEqual(
    ...
    def test_recurrence(self):
        ...
        self.assertEqual(
            dates[0], datetime.datetime(2006, 1, 26, 23, 0, tzinfo=tzutc())
        )
        self.assertEqual(
            dates[1], datetime.datetime(2006, 2, 23, 23, 0, tzinfo=tzutc())
        )
        self.assertEqual(
...
class TestChangeTZ(unittest.TestCase):
    ...
    def test_change_tz_utc_only(self):
        ...
        expected_new_dates = [
            (datetime.datetime(1999, 12, 31, 17, 59, 59, 0, tzinfo=new_tz), dates[0][1])
        ]
